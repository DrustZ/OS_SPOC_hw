    ...
    def TempSafeCheckAfterRelease(self):
        ...
        for i in range(len(self.need)):
            if self.finished[i] == False:
                tmp = True
                check = False
                for j in range(len(self.RESOURCE)):
            ...
            for i in arr:
                if self.finished[i] == False:
                    ...
                    if self.ExecuteProcess(i):
                        ...
                        #check if at least one request can be done after previous process done. 
                        #not check whole sequances.
                        #if every element of Requests can't accepted after previous process done, 
                        #this mean it is not safe state
