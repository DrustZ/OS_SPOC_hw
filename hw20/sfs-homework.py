...
class fs:
    ...
    # DONE
        ...
        if self.inodes[inum].getRefCnt() == 0:
    ...
    # DONE
        ...
        addr = self.inodes[pinum].getAddr()
        if self.data[addr].getFreeEntries() == 0 or self.data[addr].dirEntryExists(newfile):
            return -1
    ...
    # DONE
        pin = self.nameToInum[parent]
        adr = self.inodes[pin].getAddr()
        if self.data[adr].getFreeEntries() == 0 or self.data[adr].dirEntryExists(newfile): 
            return -1;
        ...
        if ftype == 'd':
            newdata = self.dataAlloc()
            if newdata == -1:
                return -1
            self.data[newdata].setType('d')
            self.data[newdata].addDirEntry('.',  self.nameToInum[parent])
            self.data[newdata].addDirEntry('..', inum)
            self.inodes[inum].setAll('d',newdata,2);
        else: 
            self.inodes[inum].setAll('f',-1,1);
        self.inodes[pin].incRefCnt()
        self.data[adr].addDirEntry(newfile, inum)
        return inum
