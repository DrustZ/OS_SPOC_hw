...
kernel_thread(int (*fn)(void *), void *arg, uint32_t clone_flags) {
    cprintf("Create 1:\talloc trapframe (kernel_thread())\n");
    struct trapframe tf;
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    cprintf("\tCreate 2_1:\talloc a new proc_struct\n");
    proc = alloc_proc();
    cprintf("\tCreate 2_2:\tget a unique pid\n");
    proc->pid = get_pid();
    cprintf("\tCreate 2_3:\tset up kernel stack, trapframe for the child process\n");
    //    2. call setup_kstack to allocate a kernel stack for child process
    setup_kstack(proc);
    //    3. call copy_thread to setup tf & context in proc_struct
    copy_thread(proc, stack, tf);
    //    4. insert proc_struct into  proc_list
    cprintf("\tCreate 2_4:\tadd process into list and set it to RUNNABLE\n");
