...
enum {
  ...
  KERSTART= 0xc0000000, // start of kernel address space
...
setup_user_paging()
{
  ...
  pg_dir[0x100] = (int)pg_tbl[0] | PTE_P | PTE_W | PTE_U;
...
setup_kernel_paging()
{
  ...
  pg_dir[0x300] = (int)pg_tbl[0] | PTE_P | PTE_W ;
  for (i = 1; i < 16; ++i){
  ...
  for (i=0;i<1024*16;i++) pg_tbl[0][i] = (i<<12) | PTE_P | PTE_W ;  // trick to write all 4 contiguous pages
  pg_dir[0] = pg_dir[0x300];
}

mainc()
{
  pg_dir = (int *)((((int)&pg_mem) + 4095) & -4096);          // don't need low map anymore
  printf("haah\n");
